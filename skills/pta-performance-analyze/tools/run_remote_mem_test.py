...
FILTER_SCRIPT = SCRIPT_DIR / "filter_plog_memory.py"
...
def run_ascend_test(cfg, npu_script, api_name, out_dir, results):
    ...
    try:
        ...
        fn = FILTER_SCRIPT.name
        scp_upload(target, str(FILTER_SCRIPT), f"{wdir}/{fn}", env)
        log(T, f"已上传: {sn}, {fn}")
...
def main():
    ...
    if not FILTER_SCRIPT.is_file():
        sys.exit(f"错误: filter 脚本不存在 {FILTER_SCRIPT}")
